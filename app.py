...
#  * 텍스트 전처리 → TextRank+MMR 요약(세션 KB 가중) → 자연스러운 대본 리라이팅
#  * PDF/ZIP/직접붙여넣기 입력 모두 세션 KB에 누적 (용어/행동/질문)
#  * 이미지/스캔 PDF는 OCR 미지원 (경고 표기)
# ==========================================================
...
def preprocess_text_to_sentences(text: str) -> List[str]:
    ...
    for s in raw:
        ...
        if not s2:
            continue
        if re.search(r"(주요사고|안전작업방법|콘텐츠링크)$", s2):
            continue
        if len(s2) < 6:
            continue
        sents.append(s2)
...
def read_pdf_text(b: bytes) -> str:
    """PDF → 텍스트 추출 (텍스트가 너무 짧으면 이미지 PDF로 간주)"""
    try:
